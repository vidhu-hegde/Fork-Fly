...
async function searchMemories(
  ...
  const candidateVideos = rankedCandidates.slice(0, 10);
  ...
  const rankedVideos = rerankVideoMatches(videos, relevanceContext).slice(0, 10);
...
function rerankVideoMatches(videos, context) {
  ...
  const filteredVideos = scoredVideos
    .filter((entry) => entry.relevanceScore > -2)
    .map((entry) => entry.video);

  return filteredVideos.length
    ? filteredVideos
    : scoredVideos.slice(0, 10).map((entry) => entry.video);
}
