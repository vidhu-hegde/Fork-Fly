...
const MEMORIES_TOP_K_PER_PLATFORM = 25;
const MAX_SOURCE_VIDEOS = 10;
...
async function searchMemories(
  ...
  const relevanceContext = buildVideoRelevanceContext(destination, focus);
  ...
  logger.info("memories request started", {
    ...
    requestedTopK: MEMORIES_TOP_K_PER_PLATFORM,
  });
  ...
  const rankedCandidates = rerankVideoMatches(
    platformResults.flat(),
    relevanceContext
  );
  const candidateVideos = rankedCandidates.slice(0, MAX_SOURCE_VIDEOS);

  logger.info("memories rerank completed", {
    totalMatches: platformResults.flat().length,
    keptMatches: candidateVideos.length,
    droppedMatches: Math.max(0, platformResults.flat().length - rankedCandidates.length),
    destinationTokens: relevanceContext.destinationTokens,
    focusTokens: relevanceContext.focusTokens,
  });
  ...
  const rankedVideos = rerankVideoMatches(videos, relevanceContext).slice(
    0,
    MAX_SOURCE_VIDEOS
  );
  ...
  logs.push(
    rankedVideos.length
      ? `Collected ${rankedVideos.length} source videos.`
  ...
  return { videos: rankedVideos, logs };
}
...
async function searchPublicPlatform(platform, query, logger) {
  const payload = {
    ...
    top_k: MEMORIES_TOP_K_PER_PLATFORM,
...
function rerankVideoMatches(videos, context) {
  ...
  const scoredVideos = videos
    .map((video) => ({
      video,
      relevanceScore: scoreVideoMatch(video, context),
    }))
    .sort(
      (left, right) =>
        right.relevanceScore - left.relevanceScore ||
        (right.video.score || 0) - (left.video.score || 0) ||
        (right.video.viewCount || 0) - (left.video.viewCount || 0)
    );
...
function scoreVideoMatch(video, context) {
  const text = normalizeSearchText(
    [video.title, video.description, video.platform].filter(Boolean).join(" ")
  );

  let score = 0;
  const memoriesScore = Number(video.score) || 0;

  for (const token of context.destinationTokens) {
    if (text.includes(token)) {
      score += 4;
    }
  }

  for (const token of context.focusTokens) {
    if (text.includes(token)) {
      score += 2;
    }
  }

  for (const token of context.foodTokens) {
    if (text.includes(token)) {
      score += 2;
    }
  }

  for (const token of context.offTopicTokens) {
    if (text.includes(token)) {
      score -= 4;
    }
  }

  if (context.destinationTokens.length > 0) {
    const hasDestination = context.destinationTokens.some((token) =>
      text.includes(token)
    );
    if (!hasDestination) {
      score -= 3;
  ...
  const hasFoodSignal = context.foodTokens.some((token) => text.includes(token));
  if (!hasFoodSignal) {
    score -= 2;
  }

  return score + memoriesScore * 0.25;
}

function buildVideoRelevanceContext(destination, focus) {
  return {
    destinationTokens: tokenizeSearchText(destination),
    focusTokens: tokenizeSearchText(Array.isArray(focus) ? focus.join(" ") : ""),
    foodTokens: [
      "food",
      "eat",
      "eats",
