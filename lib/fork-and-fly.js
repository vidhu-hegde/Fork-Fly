const MEMORIES_ENDPOINT =
  "https://api-tools.memories.ai/agents/video-searching-api";
...
async function searchMemories(
  ...
  const response = await fetch(MEMORIES_ENDPOINT, {
    ...
    headers: {
      ...
      "X-API-Key": process.env.MEMORIES_API_KEY,
    },
    ...
    for (const rawLine of lines) {
      ...
      if (step && logs[logs.length - 1] !== step) {
        ...
        logger.info("memories stream step", {
          step,
          eventType: currentEvent || parsed.type || null,
          final:
            parsed.final === true ||
            parsed.type === "final" ||
...
function mapRecency(recency) {
  const map = {
    ...
    past_3_months: "past_month",
    past_year: "past_year",
  };

  return map[recency] || "past_year";
}
